...
My showcases usually follow the same overarching scheme - so if you haven't seen one of them you
can basically expect the same simple application in each of them.
...
It consists of easy to understand steps like `given`, `when` and `then` and also allows other
niceties like example tables in a really easy format.
