...
to get this running.
Under the hood, [Karate][] uses a full-fledged [html engine][] and provides built-ins and keywords
...
Instead of sending a real request to our backend, this time we want to verify the internal logic of
our domain object this time.
Mentioned logic here is, that a todo entry should be marked as `done`, whenever the `due date` is
*after* the given `start date`. (__Honestly I also think this is a really strange requirement, but
sometimes requirements are like that.__)

To get some practice here, let's convert it to [Three R][] as well:
