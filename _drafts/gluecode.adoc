...
Naturally, a [test-driven development][] approach works well here, but there really is no hard
requirement for this.
So even if our code already exists and/or we try to cover the behavior of our existing legacy
application, there it is still beneficial to explore its behavior and even more important to create
a document, that describes what a piece of code is supposed to - supported by actual examples.

The next few examples are taken from my showcase, which follows the same overarching scheme.
...
The first use case we are going to look into is the creation of a new entry.
In order to do that, we have to pass a `title` and a `description` over to our endpoint and verify,
if we receive the `id` of a newly created entry.
