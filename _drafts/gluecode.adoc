...
Parameter types can help to convey domain information and also allow to adhere to the [DRY][]
principle by not repeating information - here how to convert input to a date.
Apart from that the second [Cucumber][] example has no more surprises - this changes with within
the next one.
