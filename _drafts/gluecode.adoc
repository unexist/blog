...
to get this running.
Under the hood, [Karate][] uses a full-fledged [html engine][] provides built-ins and keywords for
our actual tests:
...
<1> Points the internal engine to the given [url][]
<2> Update the [path][] of the current location.
<3> Define the actual body that is send in the [request][].
<4> Set the [method][] and fire the request.
<5> And finally compare whatever comes back - here the [header][].

What about another example, that relies less on materials on board?

=== Use case #2: Create a todo with start and due date

Instead of sending a real request to our backend, we just want to verify the internal logic of our
domain object this time.
The logic here is whenever the `due date` is *after* the given `start date`, the entry should be
marked as `done`.
(__I also think this is a really strange requirement, but sometimes stakeholders are like that.__)

Converted to our [Three R][] it could be like:

----
As a user, I want to create a new todo entry that starts and ends on a given time,
, so that
with an unique ID for later retrieval.
----
