...
[line-through]#save# press btn:[:]+btn:[w].
...
----
<.> Let us mix languages here, but [zerolog] is always worth mentioning
...
Both generators create a graph based on [dot] - the plantuml version just neatly wraps the output
between `@startdot` and `@enddot`:
...
digraph tdr {
    ...
    subgraph {
        ...
        _3 [label="3. Usage of zerolog"; URL="0003-usage-of-zerolog.html"];
        _2 -> _3 [style="dotted", weight=1];
    }
  _2 -> _1 [label="Amends", weight=0]
  _3 -> _1 [label="Supersedes", weight=0]
}
@enddot
{% endplantuml %}
++++

The resulting graph includes the links between the records and when embedded as a vector graphic
([svg]) also includes links:
