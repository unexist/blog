...
links ([DRY][], you know?), introduce a new type for [technical debt][] (aptly named [TDR][]),
...
During the course of the last few years I played with the original [adr-tools][] based on the
work of its inventor [Nat Pryce][] and added some missing features.
...
Another way of generating the page is via [Maven][], which is quite handy since it is
...
[Confluence Publisher][] and put our documents to a Confluence instance of our choice.
