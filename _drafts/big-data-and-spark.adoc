...
[Apache Spark][] is probably one of the strongest contender.
...
It can make use of existing [cluster managers][] like [Hadoop YARN][], [Apache Mesos][] or
[Kubernetes][], but also supports running in standalone mode even on a single machine.
...
This powerful combination allows different data tasks starting from simple data loading, SQL
queries and up to streaming computation.
...
Spark applications can either be run in local or **client mode** in a single [JVM][] instance,
which is perfectly suited for first steps in tools like [pyspark][] or

There are multiple running modes for spark applications:

- **Local mode** or **client mode**
...
TIP: Following service writes this kind of data to the underlying HDFS store:
