...
TIP: If you want to run the examples locally here is a [Dockerfile][] for a kickstart:
<https://github.com/unexist/showcase-hadoop-cdc-quarkus/blob/master/podman/spark/Dockerfile>

=== Examples

Like every good post inside of this blog we again are going to use a simple [todo application][]
and its really simple data model.
Heavy lifting is a nice exercise for the reader, so we keep it simple and probably stupid, too:
...
Again, the standalone version is more than enough, we just need start Kafka. Since Kafka itself can
also be a pain, we will throw in [RedPanda][] as a lightweight alternative.

Installing RedPanda is also pretty painless, here is how it can be done with [Podman][] for the
existing pod named _hadoop_:
...
Next step is to actually submit the job to the master along with a bit of
configuration:
