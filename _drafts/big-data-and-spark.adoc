...
Spark applications can be run either in a **cluster mode**, which allows the usage of many worker
nodes and executors, but also feel at home in a **local mode** with all processes running on the
same machine.
