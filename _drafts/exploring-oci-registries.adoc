...
----
<.> Another media type - this time to indicate there is no config available.
...
Again we are free to pick from multiple options,

I think we got the why pretty much nailed down, so
...
$ podman run --rm -v .:/workspace --network=host \
    ...
    localhost:5000/todo-service:latest | jq .
...
...
----
<.> There are several options for verification available - we just rely on issuer and email.
<.> This states which checks were performed during the verification.
<.> Apparently this _critical_ is nothing of concern and a {redhatsigning}[default from RedHat].

Just as a negative check this is how it looks like when the verification actually fails:
