...
Struturizr follows a slightly different approach and provides its own set of elements and
relationships to establish a simple model.
And then based on this model different views can be extracted they can vary from a kind of
birds-eye view to something and ant would see.
