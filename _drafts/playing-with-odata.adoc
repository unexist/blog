...
Our service document from above included a navigational properties called Tasks
...
[source,bash]
----
$ curl -s -X PATCH --json '{"Title":"test3", "Description":"test3"}' http://localhost:8080/odata/Todos(3) | jq .
{
  "@odata.context": "$metadata#Todos",
  "ID": 3,
  "Title": "test",
...
[source,bash]
----
$ curl -s -X DELETE http://localhost:8080/odata/Todos(3) | jq .
{
  "@odata.context": "$metadata#Todos",
  "ID": 3,
  "Title": "test",
