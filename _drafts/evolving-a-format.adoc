...
[Avro][] is a entirely different beast and probably needs a bit of explanation first.
Originally designed for the special use cases of [Hadoop][], there were quickly other cases of
application, like usuage for [Kafka][] due to its small footprint of its binary base and
compression options.

The base mode of operation is a bundled and encoded form, which includes the schema along with the
actual data, which looks rather _interesting_ in hex view:
