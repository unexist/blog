...
requests of data scientists to move data to a specific endpoint of a [Data Lake][] in the
...
Later on they were augmented or replaced by Data Lakes and [Data Marts][], which did some
things differently:
...
Hadoop is an open-source [Java][] framework that manages large quantities of data and provides
means for further processing.
We are going to cover the processing of the actual data with [MapReduce][] in a follow-up
