...
[Data Warehouses] were the first attempt to store everything in vast SQL-databases with a rigid
schema.
They introduced complex requirements for scaling and made redundancy even worse, but did the job
for a while.

Later the data was split by domain of origin and this eased the operative pain a bit.
The removal of requirement of a schema at all really made the handling easier in [Data Lakes] and can be broken
further down into [Data Marts][] for more specific use cases.
