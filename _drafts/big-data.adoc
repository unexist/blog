...
complex scaling and redundancy requirements and mostly the same data schema.
That was broken apart into [Data Lakes][] for schemaless data, which basically offloaded the
refinement step to a future reader for a single domain and even finer grained into
[Data Mart][] for more specific use cases.

One funny property of data is the build trap:
