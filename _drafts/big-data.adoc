...
- No data schema is enforced on write and everything stays as-is in mostly flat-file
...
It allows to break tasks down into smaller jobs and run them close to the actual data at the
same time.
...
an optionally secondary namenode and many datanodes:
...
. If clients wish to write data they connect again to a specific datanode
. The namenodes keep track of the data and force balanced replication to different datanodes
