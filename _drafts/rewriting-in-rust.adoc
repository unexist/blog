...
:rustacean: https://rust-for-rustaceans.com/
...
{raii}[Resource acquisition is initialization] (RAII) is another programming idiom, which isn't of
any concern in C-based languages, but can turn into a problem in strict languages like Rust.
In layman's terns this just means whenever we initialize our holder, we also have to initialize
all of its members.

==== Problem

This easily turns into a problem, whenever the holding structure contains something, that must be
initialized later - like a socket connection:
...
ergonomics.
One of the easies ways is to wrap the connection in {option][Option], which can be initialized
with its default value and set later, but as I've said the ergonomics of mutatingfootnote:[I'll use
mutate instead of write onwards, because I think it better transports the intent.] something on the
inside is bothersome.

A better [line-through]#option# alternative is let one of the many
cellsfootnote:[More on this later.] handle this job.
{oncecell}[OnceCell], as the name implies, offers an easy way to basically initialize our socket
_once_ we are ready with the preparation.
...
the help of our {from-trait}[From] trait.

=== Interior mutability

Since the last problem was that easy to solve, let us face a new problem, which mainly is a
consequence of our easy approach before.

NOTE: I've never seen this problem in other languages before, nonetheless I still think this should
be relevant even for other languages.

==== Problem

We are happily carrying our reference of our let us say demi-god object around, everything can
access and read the data and many use-cases work splendidly as long this is read-only.

Now what happens, when something wants to actually wants to mutate (write) the data?

.Problematic code
[source,rust]
----
fn func_b(suntle: &mut Subtle) {
    subtle.visible_tags = Tagging::empty();
}

fn func_a(subtle: &mut Subtle) {
    func_b(subtle)?;
}
----

The compiler (or rather the borrow checker) happily reminds us, that there can only be one mutable
reference at the same time and the {aliasing}[aliasing rules] are violated:
...
<.> And then once we are happy with the result we ask the container to swap-out the content entirely.

=== Borrow checker

The next related problem is the (in)famous borrow checker and probably one of the main antagonists
of any {rustacean}[Rustacean] - new and old.

If we continue with our initial problem with a god object - why can't we just keep a mutable
reference of it all the time?
...
[source,c]
----
static void ClientBounds(SubClient *c, XRectangle *bounds, XRectangle *geom, int adjustx, int adjusty) {
... // <.>
}
...
----
<.> Nothing to see here besides some boring bound checking - next!
<.> Here we pass on a pointer to a variable to another function - totally fine in C-land.
...
----
<.> Bound checking is done here directly.
<.> The pattern is quite similar to the one before..
<.> ...with the addition the compiler wouldn't allow passing on a mutable reference to `self.geom`
