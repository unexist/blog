...
to initialize all of its members.
...
Well, simply because you can come pretty far without triggering beloved errors like
{error-499}[E0499] and grouping problems to keep a common thread is quite difficult.

Anyway, back to the topic at hand: Why can't we just keep a mutable reference of our god object
it all the time and pass it around?
...
Since the last problems were that easy to solve, let us face some of the consequences of our
previous approaches.
