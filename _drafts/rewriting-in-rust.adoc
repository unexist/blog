...
Interestingly this is again more about software design, cognitive load, coupling and also
about Rust's pragmatic way of handling mutability in contrast to other (functional) languages like
{haskell}[Haskell] as can be seen in the next code block:
...
If you don't mind trailing all those explicit `mut` statements the above code runs fine and if you
don't try to re-borrow anything the {aliasing}[aliasing rules] work in your favor.

A different story is the cognitive load and the coupling:
...
fn func_c(holder: &mut Holder) {
    holder.counter += 1;
}

fn func_d(holder: &Holder) { // <.>
    println!("counter={}", holder.counter);
}
...
----
<.> This access is just read-only, so no need for `mut` and also a promises to be side-effect free.
<.> See &#10103;!
