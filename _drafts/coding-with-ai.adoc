...
[GPT-3][] and its pricier and more capable successor[GPT-4][] among the top trending and topics.
Close by are articles about AI-assisted coding with dozens of different services and products.

[Slashdot][] alone lists three pages of tools and I had a tough time to select the two services
I want to give a spin during the course of this blog post.

I ultimately picked [TabNine][], because of its free tier and my previous experience when it came
to my attention and [Github][]'s [CoPilot][], due to its access to the vast amount of _free_
training data which by chance might include my own repositories.

So in this post I will try to bring my thoughts to paper with both my operational and my
management hat on and it is up to you to decide of which [color][] they are.
...
code and [prompt][] for recommendations based on the actual text and code we write.
...
for me as can be seen in the following animated GIFs:
