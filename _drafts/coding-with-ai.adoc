...
We train a large language model (LLM) on our available text (natural language) and source code and
[prompt][] for recommendations based on the actual code we write.
...
hat thinking and hopefully with my personal opinion limited to the conclusion part after that.
