...
And the second part explains how the same can be archived with versions on examples based in
[REST][].
...
This should be fairly is, since anything that can be parsed on the receiving end **is** a proper
format, so we just serialize (or encode) our object values separated by commas:
