...
the underlying hardware e.g. in networking or failure handling.

To circumvent this, MapReduce as a programming paradigm breaks the processing task into
...
- This data is collected in the *shuffling phase*, sorted and similar data piped to matching
*reducers*.
- And in the final *reducing phase* the *reducers* pick the sorted data up, aggregate it and send it
further on to the storage.
