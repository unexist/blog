...
And with every component in place it is time to talk about how to actually put them to use.

We briefly talked about the resource manager [YARN][] and its job scheduling capabilities in the previous article and
...
rectangle "Client" as c #lightgrey

rectangle "Resource manager" as rm #lightblue

together {
  rectangle "Node 1" as r1 {
    rectangle "Node manager" as nm1 #lightblue
    rectangle "App master" as am #red
    rectangle "Container" as co1 #red
  }

  rectangle "Node 2" as r2 {
    rectangle "Node manager" as nm2 #lightblue
    rectangle "Container" as co2 #red
  }
}
...
co1 -[#red]-> am: <color:red>Job status (2)
co2 -[#red]> am
...
Since all processing happens inside and with the help of jobs the step here is to create
