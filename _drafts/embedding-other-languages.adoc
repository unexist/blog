...
Unbeknownst to my past self, I got my first experience with extensibility in 2004, when I started my long journey
with {xlib}[Xlib].
...
The initial version just included everything in one binary, but I quickly discovered the ergonomics of re-compiling and
shipping everything together are annoying and I switched to a simple plugin system.
...
During the linking process the intermediate buildings blocks (namely obj(ect)-files and static libraries) are combined
to a final library or executable artifact:

.Linking process based on the diagram from {linker}
[plantuml]
----
rectangle "Static lib (1)" as lib1
rectangle "Obj file (2)" as obj1
rectangle "Obj file" as obj2
rectangle "Linker" as linker
rectangle "Lib (3)" as lib3
rectangle "Shared object (4)" as so
rectangle "Binary (3)" as bin
...
Shared objects are a type which can be dynamically loaded at runtime with the help of
{dlopen}[dlopen] and
{dlsym}[dlsym].
