...
****
If you haven't played with stackmachines before it might be a bit difficult to follow what it is
...
l4 -r-> res
----
<1> Call `lua_getglobal` to put the table `font` onto the stack at position
0footnote:[The numbers in brackets are the position on the stack]
<2> Call `lua_pushstring` to put the name of the desired row name on the stack at position -1
<3> Call `lua_gettable` to fetch the row by givem name from the table and put the result at stack position -1
<4> Call `lua_gettable` to fetch the value by given name from the row and put the result at stack position -1
****
