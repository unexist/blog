...
This time it is quite easy to find and even to shoot a nice screenshot of subtle, since I am using
...
Just feel free to skip the next few listings and joins us later.
...
And lastly, a quick glimpse into the battery sublet, which naturally makes use of the mentioned
DSL:
...
<.> Aww, this kind of reminds of {rust}[Rust] <3
...
methods there, which are later collected during load and called.
With metaprogramming, we actually define methods and call them from inside our config and
scripts and handle this accordingly.

The Ruby integration in subtle is quite vast, and we have to cut a few corners here and there
and follow loads of [line-through]#indirection# abstraction, but I think it might work, if we just
stick to the styles excerpt from above.

Loading the styles from the config consists of following basic building blocks:
...
<.> Take care, that the internal garbage collector doesn't wipe it
<.>
...
----
<.> Call Ruby's version of `setjmp` and `longjmp` and handle the leftovers

==== Error handling

Actually we covered this already in the previous section, another meme for the time being?
