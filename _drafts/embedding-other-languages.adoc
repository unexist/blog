...
I later replaced Lua with {Ruby}[Ruby] and this is an entirely different beast, but I basically
followed the same approach as we are going to see.

==== Time for Screenshots

Again let us start with something visual and this time it is quite easy, since I use subtle daily
on several devices and don't even have to mess with outdated builds.
