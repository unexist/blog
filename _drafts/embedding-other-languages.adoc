...
How a certain quality is best achieved depends on a plethora of things, but interestingly
according to {Wikipedia}[Wikipedia], extensibility revolves around future growth and proper design,
but also mentions scripting languages in the black box.

Stay put, we don't want to dwell on theory longer than this introduction has to be, so instead, we
fast-forward directly to extensibility and see what this is all about.


So please be my guest on my journey through my 20+ years of {FOSS}[FOSS]-history and listen to an
old man laying out different approaches of embedding scripting languages and why this is always
great idea.

.https://knowyourmeme.com/photos/930538-the-fairly-oddparents
image::meme-go_wrong.png[]
...
image::meme-few_hours_later.png[]
...
.Screenshot of {subtle}[subtle-0.7b]
...
It took me quite some time figuring out how to restore or rather re-invent what was missing.
Additionally, there was never a real usage of the project on my side, _but_ I can show off test
scripts in JavaScriptfootnote:[Adding {Obj-C}[Obj-c] to this mix is something for another post..]
...
/* --- %< --- snip --- %< --- */
----
<.> Highly ambitious as I've promised, but basically follows the same idea as subtle
<.> Well, just print some details of windows in the normal state

And another short one for the UI integration:
