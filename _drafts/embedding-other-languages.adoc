...
methods there, which were later collected during load or actually called.
With metaprogramming, we turn this around and define methods to provide an API for the scripts,
which is then called from inside our config and scripts and handle this accordingly.

The Ruby integration in subtle is quite vast and I'd like to show so many cool things in it, but
still time is precious as is attention.
So we have to cut a few corners here and there and follow loads of [line-through]#indirection#
abstraction, but I think it might work, if we just stick to the styles excerpt from above.
...
Actually we covered this already in the previous section, so nothing to be done here.
