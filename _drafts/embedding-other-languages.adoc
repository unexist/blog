...
on {linker}[linker] magic.
During the linking process the different immediate buildings blocks (namely obj(ect)-files and static libraries) are
assembled to a final executable:
...
but this can also be done dynamically at runtime with the help of
{dlopen}[dlopen] and {dlsym}[dlsym].
Theses {sharedobj}[shared objects] need to be prepared and treated specially,
