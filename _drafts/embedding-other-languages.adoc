...
The year is 2004 and your project is obviously written in {clang}[C], so the only real thing is to accept contribution
in the form of even more C.
Since compiling the whole project from scratch everytime is quite annoying, you design your own plugin system with
the good old buddies `dlopen` and `dlsym`:

.{deskbar}[Deskbar], plug.c
[source,c]
----
