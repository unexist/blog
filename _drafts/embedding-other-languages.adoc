...
Again without a big swing, during the linking process all intermediate parts (obj(ect)-files and static libraries) are
combined to a final executable binary or library:
...
The interesting part here is the mention shared objects can also be dynamically loaded at runtime with the help of
{dlopen}[dlopen] and {dlsym}[dlsym].

=== Runtime loading

Everything is in C is a bit more complicated, but the next few lines of code from deskbar demonstrate how this can
be done:
