...
reference manuals what I actually have to restore.
...
Luckily I already worked test-driven, so I can show off these test scripts written in
...
I can only guess what my plans for it were, but might be a glimpse into the future, but more on that
later, when we talk about the last project of this blog post.
Whatever the plans were, I didn't spend to much time on it and rubtle isn't polished in any sense.

So why do I mention it at all you might ask?
Within rubtle I followed a different approach we haven't covered so far.
Instead of inventing an own API, it is also entirely possible to create a
bridgefootnote:[Which probably is another API?] and expose everything:
...
next excerpert is boiled down and absolutely safe to handle:
...
| Another thing that depends on the embedder and the embeddeefootnote:[Does this exist?]
|===
...
cosmos.
It allows to create binaries from a plethora of languages and run them at mostly full speed
...
{Extism}[Extism] is such a framework and used in my latest project {subtle-rs}[subtle-rs] as a
re-write in Rust and the successor of subtle.
