...
application.
In other words our scripts defined an API, although according to rules of application.
Now with metaprogramming, we turn this a bit around and define methods to provide an API for the
scripts, which is then used inside of scripts.
...
----
<.> Call Ruby's version of `setjmp` and `longjmp` and wrap our own loaded
...
----
<.> `rb_cFile` is an internal symbol of `File` and we call its `read` method and pass our arguments
<.> And then a final eval - see we stick to the motto!
