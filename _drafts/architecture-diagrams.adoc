...
Therefore this blog post showcases [Struturizr][] along with three different ways of building up a model which
can be used to create [C4][] views:
...
And once done the resulting documentation is stowed away in rare places like a company fileshare
...
Both nammed views are part of the C4 model and represent the first two of the four (HINT: **CC**CC)
...
Creating an account isn't that attractive, as is the installation of dozens of packages, so let us pick
the third option and start [Podman][] (or any other engine if you insist).

=== Generating the model

Since we just have to download and run the container, the only thing we have to do in advance is take
the model from above and store it into a file locally.
...
and you should be greeted with something similar to this screenshot:

image::structurizr-lite_webview.png[]

The webpage itself is a simple viewer, that allows to cycle through the defined views, is also capable of editing
the diagrams directly and allows to convert and store them as `.png` or `.svg` to your disk.

[TIP]
====
Modification of the example from above isn't possible unless you disable [autolayout][] in the example.
====
