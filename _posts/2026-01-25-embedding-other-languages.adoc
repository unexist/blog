...
date: 2026-01-25 17:55 +0100
last_updated: 2026-01-25 17:55 +0100
author: Christoph Kappel
tags: showcase c rust lua ruby javascript webassembly duktape extism
categories: tech
toc: true
---
...
:testdriven: https://en.wikipedia.org/wiki/Test-driven_development
...
:toml: https://toml.io
:subtleconfig: https://github.com/unexist/subtle-rs/blob/master/subtle.toml
...
There were plenty of alternatives like {gkrellm}[GKrellM] readily available, but who in their right
mind prefers pre-built stuff over rolling your ownfootnote:[Beside crypto, never do that unless
...
messing with the infamous {autotools}[Autotools] and trying to compile old {clang}[C]-code with a
modern compiler this is as far as I gotfootnote:[..followed by kilometres of error trace I wasn't
in the mood to fix right now]:
...
<.> I can only guess what is was supposed to do, since the plugin is just an empty stub that returns `NULL`
<.> Yes, oh well...
...
And with even more luck, although the page is a bit unfinished, the file listing included
screenshots:
...
----
<.> The `:` here is used as a kind of namespace separator and should be read as a global table
...
here and do one mistake after another.

==== Time for Screenshots

This time we can keep it short and simple, since I am using it on a daily on several devices and
...
So when we finally start subtle everything comes together, and we see known pieces from other
projects before, which is more or the less entirely the same.
...
scripts to let them call it.

The Ruby integration in subtle is quite vast, and there are many cool things I'd like to show, but
...
Actually we covered this already in the previous section, so nothing to be done here and we better
hurry.
...
During the 2020s lots of weird things happened and I was forced into my own sort of crisis being stuck
with and on a {macOS}[macOS] for some years.
Needless to say the window management there totally annoyed me and I started another highly
ambitious project aptly named {touchjs}[touchjs].
...
Fixing something that isn't there is quite difficult, and it took me some time and reading
...
Although I really had fun playing with it, there has never been a real usage of the project.
Luckily I already worked {testdriven}[test-driven], so I can show off these test scripts written in
JavaScriptfootnote:[Adding {Obj-C}[Obj-c] to this mix is something for another post..]
along with some resulting shots of the Touch Bar:
...
image::touchjs-touchbar-observer.png[Barshot of touchjs (1/3)]
...
.{touchjs}[touchjs]/test/widgets.js:1
[source,javascript]
----
/* --- %< --- snip --- %< --- */
...
We could go into details here how the loading process and error handling works in Obj-C, but I
...
Around 2023 I started another pet project under the nice moniker {rubtle}[rubtle].
...
Whatever the plans were, I didn't spend too much time on it and rubtle isn't polished in any sense.

So why do I mention it at all you might ask?
Within rubtle I followed a different approach we haven't covered so far.
Instead of inventing an own API, I created bridgefootnote:[Which probably is another API?] and
allowed the scripts to interact directly with the underlying engine:

.{rubtle}[rubtle]/src/main.rs.js:99
[source,rust]
----
fn main() {
...
----
<.> Just file loading, no surprises here yet
<.> Now it is getting exciting - off to the next listing!

.{rubtle}[rubtle]/src/main.rs.js:55
[source,rust]
----
...
----
<.> Using the builder pattern was really a fight back then to me
<.> Here we are assembling an object by adding some values and methods
<.> And we register this as a global object

Compiled, ready  and armed we can feed this fancy test script into it:

.{rubtle}[rubtle]/test.rs.js:55
[source,rust]
----
...
$ RUSTFLAGS=-Awarnings cargo run -- ./test.js
   Compiling rubtle-duktape v0.1.0 (/home/unexist/projects/rubtle/rubtle-duktape) # <.>
   Compiling rubtle-lib v0.1.0 (/home/unexist/projects/rubtle/rubtle-lib) # <.>
   Compiling rubtle v0.1.0 (/home/unexist/projects/rubtle/rubtle)
    Finished `dev` profile [unoptimized + debuginfo] target(s) in 2.03s
     Running `target/debug/rubtle ./test.js`
<JS> "6" # <.>
----
<.> The required Rust-C-bindings, a courtesy of {bindgen}[bindgen]
<.> This contains the heave and {unsafe}[unsafe] lifting we are going to see next
<.> Well, 6, yes?

==== Error handling

Inside rubtle-lib is lots of scary stuff and I don't want to scare away my dear readers, so the
next excerpt is boiled down and absolutely safe to handle:

.{rubtle}[rubtle]/rutle-lib/src/rubtle.rs:291+777
[source,rust]
----
impl Rubtle {
    /* --- %< --- snip --- %< --- */
...
----
<.> Did I mention duktape is also a stack machine and exposes this type of API?
<.> This is a similar handling of the stack like we've seen in Lua
<.> And we essentially provide a error handler to trap errors when they occur
...
| Absolutely, there usually are many bindings and they can also be created with {ffi}[FFI]
| Depends a bit on the language, but ranges from easy to complex
| Another thing that depends on the embedder and the embeddeefootnote:[Does this word exist?]
|===

== Webassembly

I think {webassembly}[Webassembly] is one of the more interesting topics from the web technology
cosmos.
It allows to create binaries from a plethora of languages and to run them mostly at full speed
directly inside stack-basedfootnote:[Again a stack machine?] virtual machines.
Originally meant for embedding in the web, it can also be utilized in other types of software
and provide more flexibility when required, but also raw speed on execution.

There is lots of movement and things might [line-through]#break# change quite often, but
frameworks provide stability here where required.
{Extism}[Extism] is such a framework and also the one used in my latest project
{subtle-rs}[subtle-rs] as a re-write in Rust and the spiritual successor of subtle.

=== Screenshots first

subtle-rs is under active development and therefore a piece of a cake to demonstrate it:

.Screenshot of {subtle-rs}[subtle-rs-0.1.0]
image::subtle-rs-0.1.0.png[screenshot of subtle-rs]

=== Runtime loading

In contrast to the other projects, subtle-rs doesn't use a scripting language as its config, but
relies on a simple {toml}[TOML] file.
Therefore it doesn't make sense to go into detail here.
If you still are curious just check the repository: <{subtleconfig}>

Startup and loading the four existing plugins works like a charm:

.Running of {subtle-rs}[subtle-rs-0.1.0]
[source,shell]
----
...
[2026-01-25T15:48:20Z INFO  subtle_rs::plugin] Loaded plugin (time) # <.>
[2026-01-25T15:48:20Z INFO  subtle_rs::plugin] Loaded plugin (fuzzytime) #<.>
[2026-01-25T15:48:20Z INFO  subtle_rs::plugin] Loaded plugin (mem) # <.>
[2026-01-25T15:48:20Z INFO  subtle_rs::plugin] Loaded plugin (battery) # <.>
[2026-01-25T15:48:20Z INFO  subtle_rs::screen] Running on 1 screen(s)
----
<.> Written in {zig}[Zig] - <https://github.com/unexist/subtle-rs/tree/master/plugins/time>
<.> Written in Go - <https://github.com/unexist/subtle-rs/tree/master/plugins/fuzzytime>
<.> Written in JavaScript -  <https://github.com/unexist/subtle-rs/tree/master/plugins/mem>
<.> Written in Rust - <https://github.com/unexist/subtle-rs/tree/master/plugins/battery>

Under the hood the integration works a bit different to the embeddings before.
...
.{subtle-rs}[subtle-rs]/src/plugin.rs:64+84
[source,rust]
----
    /* --- %< --- snip --- %< --- */
    ...
    /* --- %< --- snip --- %< --- */
----
<.> The macro `host_fn!` allows us to define functions for our webassembly guest
<.> Funny how the path of the acpi interface has changed over the years
<.> Extism also provides an easy-to-use loader
<.> Time to register our host function

And just to complete the usual triplet again, here is what the battery plugin actually does:

.{subtle-rs}[subtle-rs]/plugins/battery/src/lib.rs
[source,rust]
----
...
----
<.> This imports the function from the host
<.> Mark this function for export to the host
<.> Sadly the `unsafe` here is required...
<.> Pretty straight forward - parse and convert with a bit error checking - one line

=== Error handling

Due to the isolation of the plugins the error handling happens inside the virtual machine:

.{subtle-rs}[subtle-rs]/src/plugins.rs:118
[source,rust]
----
    /* --- %< --- snip --- %< --- */
impl Plugin {
...
----
<.> Just a quick call and result check of the plugin function
...
Time for a conclusion after such a marathon through many ideas, languages and projects, so we can
call this a day.
...
handled entirely by your framework.

I think taken with care the integration of scripting languages can be a great way to ease the
...
languages with the ease-of-use flow of scripting.
